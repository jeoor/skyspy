...
2. **构造候选连接边**  
   使用 Delaunay 风格候选边，从所有岛屿中心生成一批较自然的邻接关系。它服务于浮岛连接，不等同于接入完整通用 Delaunay 三角剖分库。
...
- `FogOfWarSystem` 使用玩家周围的圆形 tile 半径设置 `visible/explored`。
- `WorldRenderer` 在渲染阶段叠加白色雾层，让远处区域逐渐被白雾吞没。

它不是递归视线遮挡。原因是当前地图没有传统墙体，遮挡重点不是“墙后不可见”，而是“距离越远越空白”。
...
| 为什么不用旧式地牢房间 | 项目设定是白雾浮岛，墙体地牢会和美术/剧情冲突 |
