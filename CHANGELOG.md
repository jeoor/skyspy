...
> 说明：`DungeonForge` 是项目早期名称，后续项目方向已转为 `Sky Spy（空谍）`。本文保留从早期地牢原型到当前白雾浮岛 Roguelite 的迭代记录；当前设定以 `SKY_SPY_STORY_PLAN.md` 和 `SKYSPY_FULL_PLAN.md` 为准，当前实现符合度以 `SKY_SPY_COMPLIANCE_AUDIT.md` 为准。
...
### 背景
调研报告指出低成本高反馈的视觉效果值得补充。当前游戏已有屏震、飘字和音效，但命中、开箱、拾取、升级等事件缺少瞬时视觉爆点。
